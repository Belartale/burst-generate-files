...
First the `../` option to climb to the top directory. Second the `./` option to generate files in current directory. Third the `# Create new folder` option to create new folder by `stringsReplacers`. And other options are folders in current directory.
...
Then, we can select type of string replacements.
